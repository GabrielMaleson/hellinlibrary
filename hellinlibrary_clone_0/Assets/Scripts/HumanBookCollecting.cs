...
public class HumanBookCollecting: MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
{
    if (stream.IsWriting)
    {
        // Envia dados para os outros clientes
        stream.SendNext(currentBooks);  // Envia o número de livros
        stream.SendNext(humanSpeed);    // Envia a velocidade do jogador
        stream.SendNext(transform.position);  // Envia a posição do jogador
        stream.SendNext(_nickname);  // Envia o nickname do jogador
    }
    else
    {
        // Recebe dados dos outros clientes
        currentBooks = (int)stream.ReceiveNext();  // Recebe o número de livros
        humanSpeed = (float)stream.ReceiveNext();  // Recebe a velocidade do jogador
        networkPosition = (Vector3)stream.ReceiveNext();  // Recebe a posição do jogador
        _nickname = (string)stream.ReceiveNext();  // Recebe o nickname do jogador
